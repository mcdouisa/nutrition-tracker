...
export function checkProgressiveOverload(programId, dayId) {
  ...
  current.exercises.forEach(curEx => {
    const progEx = day.exercises.find(e => e.name === curEx.name)
    if (!progEx) return
    const prevEx = previous.exercises.find(e => e.name === curEx.name)
    if (!prevEx) return

    if (!didHitMaxReps(curEx, progEx)) return
    if (!didHitMaxReps(prevEx, progEx)) return

    // Use the weight logged in the most recent session
    const weightStr = curEx.sets.find(s => s.type === 'working' && s.weight && s.weight !== 'BW')?.weight
    if (!weightStr) return
    const weight = parseFloat(weightStr)
    if (isNaN(weight)) return

    const inc = getProgressiveIncrement(curEx.name)
    bumps.push({ name: curEx.name, oldWeight: weight, newWeight: weight + inc, increment: inc })
  })
