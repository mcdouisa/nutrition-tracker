...
import { doc, getDoc, setDoc, collection, getDocs, writeBatch, onSnapshot, addDoc, updateDoc, query, orderBy } from 'firebase/firestore'
...
export async function updateFeedbackItem(feedbackId, updates) {
  ...
  try {
    ...
    if (updates.status === 'resolved'    && !enriched.resolvedAt) enriched.resolvedAt = now
