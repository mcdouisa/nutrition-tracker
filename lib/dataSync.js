...
import { doc, getDoc, setDoc, collection, getDocs, writeBatch, onSnapshot, addDoc, updateDoc, deleteDoc, query, orderBy, limit, collectionGroup, where } from 'firebase/firestore'
...
// Returns true if data has at least one nutrition metric with a real value
function hasNutritionalData(data) {
  if (!data?.nutritionMetrics) return false
  return data.nutritionMetrics.some(m => (m.value || 0) > 0)
}

// Returns the sum of all nutrition metric values in a dataset
function nutritionSum(data) {
  if (!data?.nutritionMetrics) return 0
  return data.nutritionMetrics.reduce((sum, m) => sum + (m.value || 0), 0)
}

// Returns true if data has any logged food entries
function hasHistoryEntries(data) {
  return Array.isArray(data?.nutritionHistory) && data.nutritionHistory.length > 0
}

// Returns true if data has any checked/completed checklist items
function hasChecklistProgress(data) {
  if (!Array.isArray(data?.checklistItems)) return false
  return data.checklistItems.some(item => item.checked || (item.count || 0) > 0)
}
...
export async function saveHistoryEntry(userId, date, data) {
  ...
  try {
    ...
    // --- Safeguard 1: Date consistency check ---
    // If the data has a date field and it doesn't match the target date, something is wrong.
    if (data.date && toLocalDateStr(data.date) !== dateStr) {
      console.error(
        `[DataGuard] Blocked save: data.date (${data.date}) does not match target date (${dateStr}). Skipping to prevent data corruption.`
      )
      return false
    }
    ...
    // --- Safeguard 2: Read-before-write for past days ---
    const today = toLocalDateStr()
    if (dateStr !== today) {
      ...
      if (existingSnap.exists()) {
        ...
        // Block if we'd be overwriting real values with zeros
        if (hasNutritionalData(existing) && !hasNutritionalData(data)) {
          console.error(
            `[DataGuard] Blocked save: existing data for ${dateStr} has nutrition values but incoming data has none. Skipping to prevent data loss.`,
            { existing: nutritionSum(existing), incoming: nutritionSum(data) }
          )
          return false
        }

        // --- Safeguard 3: Auto-backup before overwriting non-empty past day data ---
        // Covers metrics, logged food entries, and checklist progress — not just
        // metric totals — so a day with real content is never overwritten without
        // a recovery copy, even if the write itself isn't blocked.
        if (hasNutritionalData(existing) || hasHistoryEntries(existing) || hasChecklistProgress(existing)) {
          const backupRef = doc(
            db, 'users', userId, 'backups',
            `${dateStr}_${Date.now()}`
          )
...
export async function completeOnboarding(userId, onboardingData) {
  ...
  try {
    ...
    await setDoc(profileRef, {
      ...
      onboardingProfile: onboardingData.profile,
      updatedAt: new Date().toISOString()
    }, { merge: true })
    return true
  } catch (error) {
...
// Dismiss a notification. Pass autoExpired=true when expiring old ones so they
// can still be retrieved for the "Catch Up" view.
export async function dismissNotification(userId, notificationId, autoExpired = false) {
  if (!isConfigured || !db) return false

  try {
    const docRef = doc(db, 'users', userId, 'notifications', notificationId)
    const update = { dismissed: true }
    if (autoExpired) update.autoExpired = true
...
// Load auto-expired announcements for the "Catch Up" view (announcements > 7 days old)
export async function loadArchivedAnnouncements(userId) {
  if (!isConfigured || !db || !userId) return []
  try {
    const snap = await getDocs(
      query(collection(db, 'users', userId, 'notifications'), where('autoExpired', '==', true))
    )
    return snap.docs
...
export async function loadAllActivityData(limitDays = 30) {
  ...
  try {
    ...
    return snap.docs.map(d => ({
      ...
      updatedAt: d.data().updatedAt  // ISO timestamp for time-of-day analytics
    }))
  } catch (error) {
...
// ── Admin management ───────────────────────────────────────────────────────
export async function setUserAdminStatus(targetUserId, isAdmin) {
  if (!isConfigured || !db) return false
  try {
    await setDoc(doc(db, 'userProfiles', targetUserId), { isAdmin }, { merge: true })
    return true
  } catch (e) { console.error('Error setting admin status:', e); return false }
}

// ── Body weight tracking ───────────────────────────────────────────────────
export async function saveBodyWeight(userId, dateStr, weight, unit = 'lbs') {
  if (!isConfigured || !db || !userId) return false
  try {
    await setDoc(doc(db, 'users', userId, 'bodyWeight', dateStr), {
      weight: parseFloat(weight), unit, date: dateStr,
      loggedAt: new Date().toISOString()
    })
    return true
  } catch (e) { console.error('Error saving body weight:', e); return false }
}

export async function loadBodyWeightHistory(userId, limitDays = 90) {
  if (!isConfigured || !db || !userId) return []
  try {
    const snap = await getDocs(
      query(collection(db, 'users', userId, 'bodyWeight'), orderBy('date', 'desc'), limit(limitDays))
    )
    return snap.docs.map(d => d.data()).sort((a, b) => a.date.localeCompare(b.date))
  } catch (e) { console.error('Error loading body weight:', e); return [] }
}
